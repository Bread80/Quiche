    ...
    case ParamIndex of
      ...
      begin
        ...
        if Loading and (Prim <> nil) then
          CheckType := GetSystemType(Prim.LType)
      end;
      2:
      begin
        Param := @ILItem.Param2;
        if Loading and (Prim <> nil) then
          CheckType := GetSystemType(Prim.RType);
      end;
      3:
      begin
        Param := @ILItem.Param3;
        if Loading and (Prim <> nil) then
          CheckType := GetSystemType(Prim.ResultType);
      end;
    else
      System.Assert(False);
    end;
        ...
        if MoveState[Reg].Param <> nil then
          ...
          begin
            ...
            if MoveState[Reg].CheckType <> nil then
              ...
              ToType := MoveState[Reg].Param.GetUserType;
