...
type TBaseReader = class
  ...
  public
    ...
    //Skips all whitespace including new lines. Returns True if we're ae EOF
    function SkipWhiteNL: Boolean;
  end;
...
function TBaseReader.GetEOF: Boolean;
begin
  Result := FEOF or (FLineNo > FLines.Count);
end;
...
begin
  Result := not EOF and (FColumn < Length(FLine));
  if Result then
  begin
