...
begin
{$ifdef fpc}
  FindAllDirectories(List, ConcatPaths([QuicheFolder, PlatformsBaseFolder]), False, ';');
{$else}
...
begin
{$ifdef fpc}
  Result := ConcatPaths([QuicheFolder, 'Tests']);
{$else}
...
begin
  ...
  begin
{$ifdef fpc}                                                    
    LoadErrorData(ConcatPaths([QuicheFolder, ErrorsFilename]));
{$else}
    LoadErrorData(TPath.Combine(QuicheFolder, ErrorsFilename));
{$endif}
    InitialiseOperators;
{$ifdef fpc}
    LoadOperatorsFile(ConcatPaths([QuicheFolder, OperatorsFilename]));
{$else}
    LoadOperatorsFile(TPath.Combine(QuicheFolder, OperatorsFilename));
{$endif}
    InitialiseFragments;
{$ifdef fpc}
    LoadFragmentsLibrary(ConcatPaths([QuicheFolder, FragmentsFilename]));
{$else}
    LoadFragmentsLibrary(TPath.Combine(QuicheFolder, FragmentsFilename));
{$endif}
    InitialisePrimitives;
{$ifdef fpc}
    LoadPrimitivesFile(ConcatPaths([QuicheFolder, PrimitivesFilename]));
{$else}
...
{$ifdef fpc}
  LoadIntrinsicsFile(ConcatPaths([QuicheFolder, IntrinsicsFilename]));
{$else}
